...
@pytest.mark.asyncio
async def test_send_otp_valid_email(client):
    """Send OTP to a valid email should return 200."""
...
async def test_send_otp_invalid_email(client):
    """Send OTP with invalid email should return 422."""
    response = await client.post("/auth/send-otp", json={"email": "invalid"})
...
async def test_verify_otp_wrong_otp(client):
    ...
    response = await client.post("/auth/verify-otp", json={"email": "test@example.com", "otp": "000000"})
