...
async def verify_gps_flow():
    ...
    async with async_session() as db:
        try:
            ...
            lab = Lab(id=lab_id, name=f"Lab {u_id}", slug=f"slug-{u_id}", 
                      phone=l_phone, city="Mumbai", address="GPS HQ", lat=0, lng=0)
            db.add(lab)
            
            patient = User(id=user_id, name=f"Patient {u_id}", phone=p_phone, role="user")
            db.add(patient)
            
            tech_user = User(id=tech_uid, name=f"Tech {u_id}", phone=t_phone, role="technician", lab_id=lab_id)
            db.add(tech_user)
            await db.flush()
            
            tech = Technician(id=tech_id, user_id=tech_uid, lab_id=lab_id, name=f"Tech {u_id}", phone=t_phone, 
                              shift=ShiftType.morning, status=TechnicianStatus.idle)
            db.add(tech)
            
            booking = Booking(id=booking_id, user_id=user_id, lab_id=lab_id, tech_id=tech_id, 
                             type=BookingType.home_collection, scheduled_at=datetime.now(timezone.utc), 
                             patient_name=f"Patient {u_id}", patient_phone=p_phone, total_amount=1000, 
            ...
            tech_context = {"sub": str(tech_uid), "technician_id": str(tech_id), "role": "technician"}
