...
def upgrade() -> None:
    ...
    # 2. Add new columns
    op.add_column('reports', sa.Column('lab_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('labs.id'), nullable=True))
    op.add_column('reports', sa.Column('uploaded_by_tech_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('technicians.id'), nullable=True))
    ...
    op.add_column('reports', sa.Column('file_size_bytes', sa.Integer(), nullable=False, server_default='0'))
