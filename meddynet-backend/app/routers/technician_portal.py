...
async def get_tech_stats(
    ...
    completed_res = await db.execute(select(func.sum(Booking.total_amount)).filter(Booking.technician_id == tech_id, Booking.status == BookingStatus.completed))
    total_earnings = completed_res.scalar() or 0

    # 2. Daily Earnings (today)
    today_start = datetime.now(timezone.utc).replace(hour=0, minute=0, second=0, microsecond=0)
    today_res = await db.execute(select(func.sum(Booking.total_amount)).filter(Booking.technician_id == tech_id, Booking.status == BookingStatus.completed, Booking.created_at >= today_start))
    ...
    completed_count_res = await db.execute(select(func.count(Booking.id)).filter(Booking.technician_id == tech_id, Booking.status == BookingStatus.completed))
...
async def get_my_jobs(current_user: dict = Depends(require_role(["technician"])), db: AsyncSession = Depends(get_db)):
    ...
    query = select(Booking).filter(
        ...
        Booking.status.in_([BookingStatus.assigned, BookingStatus.on_the_way, BookingStatus.arrived, BookingStatus.sample_collected])
...
async def get_available_jobs(current_user: dict = Depends(require_role(["technician"])), db: AsyncSession = Depends(get_db)):
    """
...
async def self_assign_job(booking_id: str, current_user: dict = Depends(require_role(["technician"])), db: AsyncSession = Depends(get_db)):
    tech_id = current_user.get("technician_id")
    if not tech_id:
        tech_res = await db.execute(select(Technician).filter(Technician.user_id == current_user["sub"]))
        tech = tech_res.scalar_one_or_none()
