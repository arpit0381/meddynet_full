...
async def create_booking(
    ...
    try:
        ...
        background_tasks.add_task(notification_service.send_booking_confirmation, payload.patient_phone, str(new_booking.id))
        ...
        test_summaries = [{"id": str(t.id), "lab_test_id": str(t.lab_test_id), "price_at_booking": t.price_at_booking} for t in new_booking.tests]
        booking_data = {
...
@router.get("")
@router.get("/me")
async def get_my_bookings(skip: int = 0, limit: int = Query(default=50, le=200), current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    if booking_ids:
        pay_res = await db.execute(select(Payment.booking_id, Payment.status).filter(Payment.booking_id.in_(booking_ids)))
...
async def get_booking_detail(booking_id: str, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def cancel_booking(booking_id: str, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def track_phlebotomist(booking_id: str, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    try:
    ...
    query = select(Booking, Technician).join(Technician, Booking.technician_id == Technician.id).filter(
        Booking.id == b_id,
