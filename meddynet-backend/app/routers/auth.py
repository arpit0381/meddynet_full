...
from app.schemas.auth import (
    ...
    RefreshTokenRequest, LoginRequest, RegisterRequest, LabOnboardingRequest
)
from app.models.user import User
from app.models.lab import Lab
from app.models.payment import LabWallet
from app.services.auth_service import send_otp, verify_otp, create_access_token, create_refresh_token, blacklist_token, verify_password, get_password_hash
...
async def register_route(payload: RegisterRequest, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(User).filter((func.lower(User.email) == func.lower(payload.email)) | (User.phone == payload.phone_number)))
...
async def register_lab(payload: LabOnboardingRequest, db: AsyncSession = Depends(get_db)):
    ...
    if target_user:
        if target_user.role in ['lab_admin', 'lab_staff', 'lab', 'admin']:
             ...
             raise HTTPException(status_code=400, detail=f"{field} already registered with an active Lab account.")
...
async def verify_otp_route(payload: VerifyOTPRequest, db: AsyncSession = Depends(get_db)):
    try:
        ...
        raise HTTPException(status_code=400, detail="OTP verification failed. Please check and try again.")
...
async def login(request: Request, payload: LoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    if prefs.get("two_fa_enabled"):
        ...
        import time
        temp_payload = {"sub": str(user.id), "type": "2fa_challenge", "exp": int(time.time()) + 300}
...
async def verify_password_endpoint(payload: VerifyPasswordRequest, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def enable_2fa(current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    otp = await send_otp(user.phone)
    response = {"message": "OTP sent to your registered phone number", "phone": user.phone[-4:]}
    # FIX 10: OTP logged server-side only, never in HTTP response
    from app.config import settings
    if settings.ENVIRONMENT == "development":
...
async def verify_2fa(payload: TwoFAVerifyRequest, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
