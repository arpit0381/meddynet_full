...
async def create_order(payload: PaymentCreate, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logger.error(f"Payment initiation failed for booking {payload.booking_id}: {e}", exc_info=True)
...
@router.post("/deposit")
async def create_deposit(payload: DepositCreate, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def verify_payment(payload: PaymentVerify, db: AsyncSession = Depends(get_db)):
    ...
    if payment.booking_id:
        ...
        await update_lab_wallet_on_payment(payment.lab_id, payment.total_amount, payment.commission_amount, booking.id, db)
    else:
        # User Wallet Top-up
...
async def get_payment_detail(payment_id: str, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Fetch a single payment detail."""
    result = await db.execute(
        select(Payment).filter(Payment.id == uuid.UUID(payment_id), Payment.user_id == uuid.UUID(current_user["sub"]))
    )
