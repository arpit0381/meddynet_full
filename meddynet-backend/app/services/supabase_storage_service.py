...
class SupabaseStorageService:
    ...
    async def upload_report(self, file: UploadFile, user_id: str, booking_id: str):
        ...
        try:
            ...
            res = supabase_admin.storage.from_(self.reports_bucket).create_signed_url(
                file_path, 315360000
            )
            ...
            logger.error(
                f"Supabase report upload failed for user={user_id}, booking={booking_id}: {e}"
            )
            return None
    ...
    async def upload_profile_image(self, file: UploadFile, user_id: str):
        ...
        try:
            ...
            res = supabase_admin.storage.from_(self.profiles_bucket).get_public_url(
                file_path
            )
            ...
            logger.error(
                f"Supabase profile image upload failed for user={user_id}: {e}"
            )
            return None
    ...
    async def upload_health_record(self, file: UploadFile, user_id: str):
        ...
        try:
            ...
            logger.error(
                f"Supabase health record upload failed for user={user_id}: {e}"
            )
            return None
